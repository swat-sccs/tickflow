...
export default async function MembersPage() {
  ...
  const totalLeads   = members.filter(m => m.projectMembers.some(pm => pm.role === "lead")).length;
  const allTasks     = members.flatMap(m => m.taskAssignees.map(ta => ta.task));
  const globalOpen   = allTasks.filter(t => t.status !== "done" && t.status !== "shipped").length;
  const globalDone   = allTasks.filter(t => t.status === "done"  || t.status === "shipped").length;
  ...
  return (
    <div className="flex flex-col gap-10 px-8 py-10 w-full max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {(
          [
            { label: "Members",    value: totalMembers, Icon: Users,     accent: "#6366f1" },
            { label: "Team Leads", value: totalLeads,   Icon: Crown,     accent: "#f59e0b" },
            { label: "Open Tasks", value: globalOpen,   Icon: CircleDot, accent: "#f97316" },
            { label: "Delivered",  value: globalDone,   Icon: Trophy,    accent: "#10b981" },
          ] as const
        ).map(stat => (
          <div
            key={stat.label}
          ...
          >
            ...
            <span className="text-4xl font-black tabular-nums" style={{ color: stat.accent }}>
              {stat.value}
            </span>
            <div
              className="absolute inset-x-0 bottom-0 h-[3px]"
              style={{ background: `linear-gradient(to right, ${stat.accent}cc, transparent)` }}
            />
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
        {members.map((member, idx) => {
          ...
          const done     = tasks.filter(t => t.status === "done" || t.status === "shipped").length;
          const inprog   = tasks.filter(t => t.status === "inprogress").length;
          const blocked  = tasks.filter(t => t.status === "blocked").length;
          const queued   = tasks.filter(t => t.status === "backlog"  || t.status === "todo").length;
              ...
              <div className="flex flex-col gap-4 px-5 pt-11 pb-5">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex flex-col min-w-0 pt-0.5">
                    <span className="font-bold text-lg leading-tight truncate">{member.name}</span>
                    <span className="text-xs text-muted-foreground truncate">{member.email}</span>
                    <span className="text-[10px] text-muted-foreground/50 mt-0.5">
                      Since {member.createdAt.toLocaleDateString("en-US", { month: "short", year: "numeric" })}
                    </span>
                  </div>
                  ...
                  <div className="relative shrink-0">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                      <span className="text-sm font-bold leading-none">{done}</span>
                ...
                {total > 0 ? (
                  <div className="flex flex-col gap-1.5">
                    <div className="flex h-2 w-full rounded-full overflow-hidden">
                      ...
                      {inprog  > 0 && <div className="h-full bg-amber-400"  style={{ width: `${(inprog  / total) * 100}%` }} />}
                      {blocked > 0 && <div className="h-full bg-red-500"    style={{ width: `${(blocked / total) * 100}%` }} />}
                      {queued  > 0 && <div className="h-full bg-muted"      style={{ width: `${(queued  / total) * 100}%` }} />}
                    </div>
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px] text-muted-foreground">
                      {done    > 0 && <span className="flex items-center gap-1"><span className="size-1.5 rounded-full bg-emerald-500 inline-block"/>{done} done</span>}
                      {inprog  > 0 && <span className="flex items-center gap-1"><span className="size-1.5 rounded-full bg-amber-400 inline-block"/>{inprog} active</span>}
                      {blocked > 0 && <span className="flex items-center gap-1"><span className="size-1.5 rounded-full bg-red-500 inline-block"/>{blocked} blocked</span>}
                      {queued  > 0 && <span className="flex items-center gap-1"><span className="size-1.5 rounded-full bg-muted-foreground/40 inline-block"/>{queued} queued</span>}
                    </div>
                  </div>
                ) : (
                  <p className="text-xs text-muted-foreground/50 italic">No tasks assigned yet</p>
                )}
                ...
                {member.projectMembers.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 border-t pt-3 mt-1">
                    {member.projectMembers.map(pm => (
                      <Link key={pm.projectId} href={`/projects/${pm.project.slug}`}>
                        <Badge
