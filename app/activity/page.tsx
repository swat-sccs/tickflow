...
export default async function ActivityPage() {
  ...
  const userStats = users.map(u => {
    ...
    return {
      id: u.id,
      name: u.name,
      open:       tasks.filter(t => t.status !== "done" && t.status !== "shipped").length,
      inprogress: tasks.filter(t => t.status === "inprogress").length,
      done:       tasks.filter(t => t.status === "done" || t.status === "shipped").length,
  ...
  return (
    <div className="w-[90%] mx-auto flex flex-col gap-6 py-10">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground">Recent task activity and team workload overview.</p>
      </div>
